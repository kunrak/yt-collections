...
function downloadBackup() {
  ...
  link.href = url;
  link.download = `channel-collections-backup-${new Date()
    .toISOString()
    .slice(0, 10)}.json`;
  ...
  return collections
    .filter(
      (collection) =>
        ...
        Array.isArray(collection.channelIds),
    )
    .map((collection) => ({
      id:
      ...
      ...(typeof collection.expanded === "boolean"
...
async function importBackupData(data) {
  ...
  for (const collection of importedCollections) {
    const id = existingCollectionIds.has(collection.id)
      ? createBackupId()
      : collection.id;
