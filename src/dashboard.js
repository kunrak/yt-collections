...
function setupChannelDragDrop() {
  ...
  document.addEventListener("dragend", (e) => {
    ...
    if (channelDragParent) {
      channelDragParent
        .querySelectorAll(".collection-channel-item.drag-over")
        .forEach((el) => {
          el.classList.remove("drag-over");
        });
    }
  ...
  document.addEventListener("drop", (e) => {
    ...
    const collectionId =
      channelDragParent.closest(".collection-item")?.dataset.id;
    ...
    const newChannelIds = channelItems
      .map((item) => {
    ...
    item.innerHTML = `
      <div class="collection-item-main">
        ...
        <div class="collection-actions">
          ...
          <button class="delete-btn" data-id="${col.id}" title="Delete">×</button>
        </div>
      </div>
    ...
    for (const channelId of col.channelIds) {
      ...
      chItem.innerHTML = `
        ...
        <div class="channel-actions">
          ...
          <button class="remove-channel-btn" data-id="${ch.id}" title="Remove">×</button>
        </div>
      `;
...
function renderTopbar() {
  ...
  if (state.view === "home") {
    ...
    el("channelCount").textContent =
      `${uniqueChannelIds().length} channel${uniqueChannelIds().length === 1 ? "" : "s"}`;
  ...
  for (const v of videos) {
    ...
    card.innerHTML = `
      ...
      <div class="video-info">
        ${state.view === "home" && labels.length ? `<div class="video-collection-label">${escapeHtml(labels.join(" · "))}</div>` : ""}
